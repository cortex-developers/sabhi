...
function BioGallery() {
  ...
  const pap = bios.filter(bio => Array.isArray(bio.project) && bio.project.includes("pap"));
  const map = bios.filter(bio => Array.isArray(bio.project) && bio.project.includes("map"));
  const cap = bios.filter(bio => Array.isArray(bio.project) && bio.project.includes("cap"));
  const nas = bios.filter(bio => Array.isArray(bio.project) && bio.project.includes("nas"));
...
<Typography variant="h4" sx={{ padding: '20px',justifyContent: 'center', textAlign: 'center', fontFamily: 'Notable, sans-serif'}} gutterBottom>  <span style={{ backgroundColor: 'cyan' }}> Physical Health and Performance </span> 
</Typography>
      ...
      <Typography variant="h4" sx={{ padding: '20px',justifyContent: 'center', textAlign: 'center', fontFamily: 'Notable, sans-serif'}} gutterBottom>  <span style={{ backgroundColor: 'magenta' }}> Mental Health and Performance </span> 
</Typography>
      <Grid container spacing={2} sx={{ padding: '20px', justifyContent: 'center', marginTop: '20px', marginBottom: '20px', marginLeft: 'auto', marginRight: 'auto' }}>
        {map.map((bio, index) => (
         <Grid item xs={12} sm={6} md={2} key={index}>
